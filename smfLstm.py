...
def run_stock_prediction(ticker):
    try:
        ...
        df = df[['Close']]
        df = df.reset_index()
        df.columns = ['Date', 'Close']  # Rename columns to remove MultiIndex
        df['Date'] = pd.to_datetime(df['Date'])

        # Merge stock data with sentiment data
        company_sentiments = sentiment_df[sentiment_df['Company'] == ticker].copy()
        company_sentiments['Date'] = pd.to_datetime(company_sentiments['Date'])
        company_sentiments['Sentiment'] = company_sentiments['Sentiment'].map(sentiment_mapping)
        
        # Print DataFrame info for debugging
        print(f"\nDataFrame structure for {ticker}:")
        print("Stock data (df):")
        print(df.head())
        print("\nSentiment data:")
        print(company_sentiments.head())
        
        # Ensure both DataFrames have the same structure
        company_sentiments = company_sentiments[['Date', 'Sentiment']].copy()
        
        # Merge with main dataframe on date
        df = pd.merge(df, company_sentiments, on='Date', how='left')
        ...
        df[['Close', 'Sentiment']] = scaled_data
        ...
        test_predict = scaler.inverse_transform(np.concatenate((test_predict, np.zeros_like(test_predict)), axis=1))[:,
                       0]
        ...
        for _ in range(30):
            ...
            sentiment_variation = sentiment_weight * np.random.randn()
            next_input = np.array([[pred[0][0], pred_input[0, -1, 1] + sentiment_variation]])
        ...
        sentiment_influence = 0.05 * predicted_values
        ...
        plt.plot(range(1, 31), predicted_values, color='blue', label="Predicted Price")
        plt.title(f"{ticker} Predicted Price Range")
        plt.xlabel("Days")
        plt.ylabel("Stock Price")
        plt.legend()
        plt.grid()
...
for ticker in results:
    ...
    mbe = np.mean(test_predict - y_test)
